...
import tx02.model.StartEndDateTime;
import tx02.ude.EndOfPromotionException;
import tx02.ude.NotYetPromotedException;
import tx02.ude.ProductStockException;
import tx02.ude.UnpaidOrderAmountExceedingException;
...
public class OrderItemReviewImpl implements OrderItemReview {
	...
	/**
	 * 由
	 * OrderItemBean內的產品代號(pdid)與廣告代號(adid)到Campaign表格中取得商品價格(Campaign#productPrice)。
	 */
	@Override
	public int findProductPriceByAdidAndPdid(int adid, int pdid) {
		String sql = "SELECT productPrice FROM Campaign WHERE adid = ? and pdid = ?";
		int price = jdbcTemplate.queryForObject(sql, Integer.class, adid, pdid);
		return price;
	}

	/**
	 * 計算某項商品(以OrderItemBean物件來表示)的金額，計算公式為 findProductPriceByPdidAndAdid(adid,
	 * pdid) 乘以 quantity(該項商品的數量)。 adid: 廣告代號, OrderItemBean#getAdid() pdid:
	 * 產品代號, OrderItemBean#getPdid() quantity: 數量, OrderItemBean#getAmount()
	 */
	@Override
	...
	/*
	 * 更新客戶的未付款訂購金額。 1. 處理一項訂單明細時，該項明細的金額 +
	 * 該客戶的未付款訂購金額不能超過限額(ORDER_AMOUNT_LIMIT) 2.
	 * 到Customer表格中取出Customer#unpaid_order_amount 3.
	 * 加上本訂單明細的金額後，檢查加總後的數值是否超過限額(ORDER_AMOUNT_LIMIT)。如果超過限額，則該
	...
	public int updateUnpaidOrderAmount(OrderItemBean oib, int cmid) {
		...
		String sql = "UPDATE Customer SET unpaid_order_amount " + " = unpaid_order_amount + ? WHERE cmid = ?";
		int n = jdbcTemplate.update(sql, currentAmount, cmid);
		return n;
	}

	/**
	 * 檢查訂購之商品是否已經超過促銷期限 void checkCampaignPeriod(Date OrderBean#orderDate)
	 * 檢查訂單的時間是否介於促銷期限之內， 如果尚未開始促銷(OrderBean#orderDate <
	 * Campaign#startDateTime)， 丟出NotYetPromotedException，
	...
	/*
	 * int pdid: 產品代號 int orderedQuantity: 訂購數量 更新商品的數量：必須先檢查訂購之商品的數量是否足夠
	 * 檢查Product表格中的庫存數量(Product#stock)減去orderedQuantity 如果小於0
	 * 丟出ProductStockException， 否則 Product#stock -= orderedQuantity;
	 */
	@Override
