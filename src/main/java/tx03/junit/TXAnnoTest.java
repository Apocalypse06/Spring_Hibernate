...
import tx03.facade.OrderItemService;
import tx03.facade.OrderService;
...
public class TXAnnoTest {
    ...
    {
    	ctx = new ClassPathXmlApplicationContext("tx03/applicationContext.xml");
    	...
    	Set<OrderItemBean> items  = new HashSet(Arrays.asList(oi1, oi2, oi3, oi4));
    ...
    public void processOrderItem() {
		int cmid = 1 ;  // 客戶編號為 1 
		Date date = java.sql.Date.valueOf("2017-05-22");
    ...
    public void findProductPriceByAdidAndPdid() {
		int price ;
		for (int n = 1; n <= 4; n++) {
			price = oir.findProductPriceByAdidAndPdid(1, n);
